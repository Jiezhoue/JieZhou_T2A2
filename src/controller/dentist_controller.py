...
def dentist_login():
    try:
        ...
        if not dentist:
            return abort(400, description="username is not exist")
        elif not bcrypt.check_password_hash(dentist.password, dentist_fields["password"]):
            return abort(400, description="password is not correct")
...
def dentist_signup():
    try:
        ...
        dentist = Dentist.query.filter_by(username=dentist_fields["username"]).first()
        if dentist:
            return abort(409, description="This dentist is already registered.")
...
def make_appointment(user,id):
    try:  
        ...
        try:
            d = datetime.strptime(booking_fields["date"], '%Y-%m-%d')
        except ValueError:
            return abort(400, description="Invalid date format. Please use YYYY-MM-DD format.")
...
def close_booking(dentist,id):
    ...
    if not booking:
        return abort(404, description="booking not exist")
    if booking.dentist_id != dentist.id:
        return abort(403, description="That's not your patient")
    if booking.status == "Close":
        return abort(409, description="This booking is already closed!")
...
def add_treatment(dentist, id):
    ...
    if booking.dentist_id != dentist.id:
        return abort(403, description="That's not your patient")
    if booking.status != "Close":
        return abort(409, description="You have to close this booking to add treatment details.")
...
def delete_treatment(dentist, booking_id, treatment_id):
    ...
    if not treatment:
        return abort(404, description="This treatment is not exist")
